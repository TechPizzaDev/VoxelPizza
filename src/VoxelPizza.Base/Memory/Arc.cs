...
/// </summary>
/// <typeparam name="T">The type implementing <see cref="IDestroyable"/> to track.</typeparam>
[DebuggerDisplay($"{{{nameof(GetDebuggerDisplay)}(),nq}} ({{{nameof(_target)},nq}})")]
public sealed class Arc<T> : IArc<T>
    where T : IDestroyable
{
    ...
    public bool TryGet([MaybeNullWhen(false)] out T target)
    {
        target = _target;
        return HasTarget;
    }
